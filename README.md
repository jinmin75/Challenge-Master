...
**현재 상태: 설계 v4.1 + 로컬 코어 v0.1.** 일정 계산·이벤트 저장·반입 상태 계약을 개발용 CLI로 실행할 수 있다. 학생용 화면, 실제 PDF 변환·LLM 계정 연결은 아직 없다. 검증 명세 K01–K25의 제품 전체 시나리오와 학습효과는 검증하지 않았다.
...
같은 저장 파일로 다시 계획을 만들면 버전이 증가하고 이전 계획이 남는다. 오류로 중단된 작업은 저장 성공으로 표시하지 않는다. 기록·입력 형식과 현재 한계는 [개발용 CLI 안내](docs/local-core.md)를 따른다. 현재 저장 파일은 암호화되지 않은 평문이다. 학생 실자료를 넣지 말고 합성 자료로 검증한다.
